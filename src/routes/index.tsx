...
function Hero() {
  ...
  useEffect(() => {
    ...
    gsap.to(spans, {
      ...
      duration: 1.1, ease: "power4.out", stagger: 0.025, delay: 0.2,
    });
    if (subRef.current) {
  ...
  useEffect(() => {
    const hero = heroRef.current, dash = dashRef.current;
    if (!hero || !dash) return;
  ...
  return (
    <section ref={heroRef} className="relative min-h-screen w-full overflow-hidden pt-24">
      <div className="pointer-events-auto absolute inset-0">
        <LiquidEther colors={["#00C6FF", "#6A5CFF", "#14F195"]} mouseForce={22} cursorSize={120} resolution={0.5} autoDemo autoSpeed={0.55} autoIntensity={2.4} />
      </div>
      ...
      <div className="relative z-10 mx-auto grid max-w-7xl grid-cols-1 gap-10 px-6 pt-10 lg:grid-cols-12 lg:gap-6">
        <div className="lg:col-span-7">
          ...
          <h1 ref={headingRef} className="font-display font-black uppercase leading-[0.9] tracking-tight text-white" style={{ fontSize: "clamp(56px, 11vw, 150px)" }}>
            <div className="overflow-hidden">{renderLetters("BUILD")}</div>
            <div className="overflow-hidden">{renderLetters("THE FUTURE")}</div>
            <div className="overflow-hidden"><span className="text-gradient">{renderLetters("OF WEALTH")}</span></div>
          </h1>
          ...
          <div className="mt-14 grid max-w-lg grid-cols-3 gap-6">
            {[{ k: "$12B+", v: "AUM" }, { k: "152", v: "Countries" }, { k: "99.99%", v: "Uptime" }].map((s) => (
              <div key={s.v}>
...
function FloatingDashboard() {
  return (
    <div className="glass-strong noise relative overflow-hidden rounded-3xl p-5 shadow-[0_30px_120px_-20px_rgba(0,198,255,0.35)]">
      ...
      <div className="mt-5 h-28 w-full"><MiniChart /></div>
...
function Services() {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(cards, { y: 100, opacity: 0, rotateX: -18 }, { y: 0, opacity: 1, rotateX: 0, duration: 1, ease: "power3.out", stagger: 0.15 });
  }, []);
  return (
...
function ServiceCard({ t, d, i }: { t: string; d: string; i: string }) {
  ...
  return (
    <div ref={ref} className="svc-card gradient-border noise relative overflow-hidden rounded-3xl glass p-8 transition-transform duration-300 hover:-translate-y-2">
      <div ref={glowRef} className="pointer-events-none absolute inset-0" />
      <div className="relative">
        <div className="mb-6 grid h-16 w-16 place-items-center rounded-2xl bg-gradient-brand text-3xl text-[#03040A] shadow-[0_0_40px_rgba(0,198,255,0.4)]">{i}</div>
        <h3 className="font-display text-2xl font-bold">{t}</h3>
        <p className="mt-3 text-white/60 leading-relaxed">{d}</p>
        <div className="mt-8 flex items-center gap-2 text-sm text-white/70">Explore <span className="text-gradient">→</span></div>
      </div>
    </div>
  );
}
    ...
    <div className="pointer-events-none absolute inset-0 overflow-hidden opacity-[0.15]">
      <svg className="absolute inset-0 h-full w-full" preserveAspectRatio="none" viewBox="0 0 1000 600">
        ...
        {Array.from({ length: 14 }).map((_, i) => (
          <path key={i} d={`M0 ${100 + i * 30} Q 250 ${60 + i * 30} 500 ${100 + i * 30} T 1000 ${100 + i * 30}`} fill="none" stroke="url(#wg)" strokeWidth="1">
            <animate attributeName="d" dur={`${8 + i * 0.3}s`} repeatCount="indefinite" values={`M0 ${100 + i * 30} Q 250 ${60 + i * 30} 500 ${100 + i * 30} T 1000 ${100 + i * 30};M0 ${100 + i * 30} Q 250 ${140 + i * 30} 500 ${100 + i * 30} T 1000 ${100 + i * 30};M0 ${100 + i * 30} Q 250 ${60 + i * 30} 500 ${100 + i * 30} T 1000 ${100 + i * 30}`} />
          </path>
        ))}
      </svg>
    </div>
    ...
    <section id="showcase" className="relative overflow-hidden py-40">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 text-center">
        ...
        <div className="mt-20 grid grid-cols-2 md:grid-cols-4 gap-6">
          {[{ k: "$12B+", v: "Assets Managed" }, { k: "99.99%", v: "Uptime" }, { k: "152", v: "Countries" }, { k: "24/7", v: "Trading" }].map((s) => (
...
function OrbitCoin({ label, radius, duration, reverse }: { label: string; radius: number; duration: string; reverse?: boolean }) {
  return (
    <div className="absolute inset-0 grid place-items-center" style={{ animation: `spin-slow ${duration} linear infinite`, animationDirection: reverse ? "reverse" : "normal" }}>
      <div className="relative" style={{ width: radius * 2, height: radius * 2 }}>
        <div className="glass-strong absolute grid h-14 w-14 place-items-center rounded-full text-xs font-bold shadow-[0_0_30px_rgba(0,198,255,0.5)]" style={{ top: -28, left: "50%", transform: "translateX(-50%)" }}>
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6">
        ...
        <div className="grid gap-16 lg:grid-cols-2 items-center">
          ...
          <div className="relative flex justify-center">
            ...
            <div ref={sphereRef} className="relative h-[380px] w-[380px] rounded-full glass-strong noise shadow-[0_0_120px_rgba(0,198,255,0.4)]" style={{ background: "radial-gradient(circle at 30% 30%, rgba(255,255,255,0.35), transparent 40%), conic-gradient(from 0deg, #00C6FF, #6A5CFF, #14F195, #00C6FF)" }}>
      ...
      <div className="mx-auto grid max-w-7xl gap-12 px-6 lg:grid-cols-2 items-center">
        ...
        <form className="glass-strong gradient-border noise relative rounded-3xl p-8 space-y-4" onSubmit={(e) => e.preventDefault()}>
          ...
          <div>
            ...
            <textarea rows={4} placeholder="Tell us about your strategy…" className="w-full resize-none rounded-xl bg-white/5 border border-white/10 px-4 py-3 outline-none focus:border-[#00C6FF] transition" />
          </div>
...
function Field({ label, placeholder, type = "text" }: { label: string; placeholder: string; type?: string }) {
  return (
    <div>
      ...
      <input type={type} placeholder={placeholder} className="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3 outline-none focus:border-[#00C6FF] transition" />
    </div>
  );
}
